...
angular.module('cst.model')
  .factory('Reservoir', [function () {
    var ReservoirNode = joint.shapes.basic.Generic.extend(
      _.extend({}, joint.shapes.basic.PortsModelInterface,
        {
          markup: '<g class="rotatable"><g class="scalable">' +
                  '<rect class="body"/></g><text class="label"/>' +
  ...
  .factory('Demand', [function () {
    var DemandNode = joint.shapes.basic.Generic.extend(
      _.extend({}, joint.shapes.basic.PortsModelInterface,
        {
          markup: '<g class="rotatable"><g class="scalable">' +
                  '<rect class="body"/></g><text class="label"/>' +
  ...
  .factory('Inflow', [function () {
    var InflowNode = joint.shapes.basic.Generic.extend(
      _.extend({}, joint.shapes.basic.PortsModelInterface,
        {
          markup: '<g class="scalable"><rect class="body"/></g>' +
                  '<text class="label"/><g class="outPorts"/>',
          portMarkup: '<g class="port port<%= id %>">' +
                      '<circle class="port-body"/>' +
                      '<text class="port-label"/></g>',
  ...
  .factory('Graph', [function () {
    function validateConnection (cellViewS, magnetS, cellViewT,
                                 magnetT, end, linkView) {
      // console.log('Graph.validateConnection()');
      if (!magnetT) {
        // console.log('!magnetT');
        return false;
      }
      ...
      if (cellViewS.id === cellViewT.id) {
        // console.log('cellViewS=cellViewT');
        return false;
      }
    ...
    return {
      init: function (el) {
        ...
        // remove links without source or target (broken)
  ...
  .factory('ModelService',
    ['Reservoir', 'Demand', 'Inflow', 'Graph',
    function (reservoir, demand, inflow, graph) {
      var nodes = [];
      ...
      return {
        addReservoir: function (cfg) {
          ...
          initial.position = {
            x: initial.position.x + 50,
            y: initial.position.y + 50};
        },
